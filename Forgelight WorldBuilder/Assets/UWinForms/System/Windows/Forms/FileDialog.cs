        ...
        internal class FileRenderer : Control
        {
            ...
            public void SetDirectory(string path, bool addPrevPath = false)
            {
                ...
                if (owner.currentFilter == "*.*")
                    files = global::System.IO.Directory.GetFiles(currentPath, "*.*").Select(f => f.Substring(currentPath.Length)).ToArray();
                else
                    files = global::System.IO.Directory.GetFiles(currentPath, "*.*").Where(f => owner.currentFilter.Contains(global::System.IO.Path.GetExtension(f).ToLower())).Select(f => f.Substring(currentPath.Length)).ToArray();
                var dirs = global::System.IO.Directory.GetDirectories(currentPath).Select(f => f.Substring(currentPath.Length)).ToArray();
            ...
            {
#if UNITY_STANDALONE
                var parent = global::System.IO.Directory.GetParent(currentPath);
        ...
        internal class PathTextBox : TextBox
        {
            ...
            private class pathButton : Button
            {
                public pathButton(string path)
                {
                    ...
                    arrowButton.Text = "";
                    arrowButton.Image = global::UWinForms.Unity.UnityWinForms.GdiImages.ArrowRight;

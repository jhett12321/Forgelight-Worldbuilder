namespace WorldBuilder.Formats.Cnk
{
    ...
    public class Cnk0 : IReadableAsset
    {
        ...
        #region Structure
        //Header
        private const string MAGIC = "CNK0";
        ...
        public bool Deserialize(BinaryStream stream, AssetManager assetManager)
        {
            ...
            chunk.Version = stream.ReadUInt32();
            ...
            chunk.DecompressedSize = stream.ReadUInt32();
            chunk.CompressedSize = stream.ReadUInt32();

            //Decompression
            byte[] compressedBuffer = stream.ReadBytes((int)chunk.CompressedSize);
            ...
            {
                //Tiles
                uint tileCount = decompressedStream.ReadUInt32();
                ...
                for (int i = 0; i < tileCount; i++)
                {
                    ...
                    tile.X = decompressedStream.ReadInt32();
                    tile.Y = decompressedStream.ReadInt32();
                    tile.UnknownInt1 = decompressedStream.ReadInt32();
                    tile.UnknownInt2 = decompressedStream.ReadInt32();

                    uint ecosCount = decompressedStream.ReadUInt32();
                    ...
                    if (ecosCount > 0)
                    {
                        ...
                        for (int j = 0; j < ecosCount; j++)
                        {
                            ...
                            eco.ID = decompressedStream.ReadUInt32();

                            uint florasCount = decompressedStream.ReadUInt32();
                            ...
                            for (int k = 0; k < florasCount; k++)
                            {
                                ...
                                uint layersCount = decompressedStream.ReadUInt32();
                                ...
                                for (int l = 0; l < layersCount; l++)
                                {
                                    ...
                                    layer.UnknownUint1 = decompressedStream.ReadUInt32();
                                    layer.UnknownUint2 = decompressedStream.ReadUInt32();
                    ...
                    tile.Index = decompressedStream.ReadUInt32();
                    tile.UnknownInt3 = decompressedStream.ReadUInt32();

                    uint imageSize = decompressedStream.ReadUInt32();
                    if (imageSize > 0)
                    {
                        tile.ImageData = decompressedStream.ReadBytes((int)imageSize).ToList();
                    }

                    uint layerTexturesCount = decompressedStream.ReadUInt32();
                    if (layerTexturesCount > 0)
                    {
                        tile.LayerTextures = decompressedStream.ReadBytes((int)layerTexturesCount).ToList();
                    }
                ...
                //Unknown Data
                chunk.UnknownInt1 = decompressedStream.ReadInt32();

                uint unknownCount = decompressedStream.ReadUInt32();
                ...
                for (int i = 0; i < unknownCount; i++)
                {
                    ...
                    unknown1.Height = decompressedStream.ReadInt16();
                    unknown1.UnknownByte1 = decompressedStream.Read1Byte();
                    unknown1.UnknownByte2 = decompressedStream.Read1Byte();
                ...
                {
                    chunk.Indices.Add(decompressedStream.ReadUInt16());
                }
                ...
                for (int i = 0; i < vertCount; i++)
                {
                    ...
                    vertex.Color1 = decompressedStream.ReadUInt32();
                    vertex.Color2 = decompressedStream.ReadUInt32();
                ...
                try
                {
                    ...
                    {
                        chunk.UnknownShorts1.Add(decompressedStream.ReadUInt16());
                    }
                    ...
                    {
                        chunk.UnknownVectors1.Add(new Vector3(decompressedStream.ReadSingle(), decompressedStream.ReadSingle(), decompressedStream.ReadSingle()));
                    }
